...
def initialize_csv(csv_name):
    #need to populate phrase_key and phrase_key_index before this.
    global phrase_key
    global phrase_key_index
    global recipes
...
def generate_training_data():
    trainX = np.zeros((len(recipes), phrase_key_index), dtype=np.int)
...
def split(list):
    ...
    train = np.zeros((trainLength, dataLength), dtype=np.int)
    test = np.empty((testLength, dataLength), dtype=np.int)

    artificial_train_index = 0
    artificial_test_index = 0
    for i, recipe in enumerate(list):
        if i%10 == 0:
            test[artificial_test_index] = recipe
            artificial_test_index += 1
        else:
            train[artificial_train_index] = recipe
